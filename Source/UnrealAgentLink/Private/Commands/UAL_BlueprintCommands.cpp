...
void FUAL_BlueprintCommands::Handle_AddNodeToBlueprint(const TSharedPtr<FJsonObject>& Payload, const FString RequestId)
{
	...
	// 复用了已有节点而不是新建时置位，随响应回给调用方 —— 让它知道
	// 「这个 node_id 是本来就在图里的」，而不是以为自己刚建了一个
	bool bReusedExistingNode = false;
	...
	if (NodeTypeLower == TEXT("event"))
	{
		...
		// 同名事件节点已经存在就复用，不再建第二个。
		//
		// 上面那段 reuse_existing 只管 Branch / Select，事件节点一路无条件新建。
		// 而**每个新建的 Actor 蓝图，EventGraph 里本来就带着 BeginPlay / Tick /
		// ActorBeginOverlap 三个占位事件节点** —— 调用方想接 BeginPlay 时又建一个，
		// 图里就有了两个 BeginPlay。编译不报错，运行时两条链都会执行。
		//
		// UE 编辑器里手动放是不会出现这种情况的：再放一次它会跳到已有的那个。
		// 这里对齐编辑器的行为。真机任务评测里踩到的。
		...
		if (ExistingEvent)
		{
			NewNodeBase = ExistingEvent;
			bReusedExistingNode = true;
		}
		else
		{
	...
	if (bReusedExistingNode)
	{
		// 调用方要能区分「刚建的」和「本来就在」。不说的话它会以为图里
		// 多了个节点，据此做出错误的后续判断
		Result->SetBoolField(TEXT("reused"), true);
		Result->SetStringField(
			TEXT("note"),
			TEXT("图中已存在同名事件节点，已复用它而不是新建 —— 蓝图里同一个事件只能有一个。"));
	}
...
)
{
	...
	if (TypeLower == TEXT("event"))
	{
		...
		// 同名事件已存在就复用 —— 和 blueprint.add_node 里同一条规则。
		//
		// 两个入口都得管：新建的 Actor 蓝图本来就带着 BeginPlay / Tick /
		// ActorBeginOverlap 三个占位事件节点，这里再建一个就成了两个 BeginPlay。
		// 编译不报错，运行时两条链都会跑。只修了 add_node 而漏掉这里的话，
		// 模型走声明式建图那条路时照样会踩 —— 真机评测里就是这么复现的。
		for (UEdGraphNode* Node : Graph->Nodes)
		{
			UK2Node_Event* Existing = Cast<UK2Node_Event>(Node);
			if (Existing && Existing->EventReference.GetMemberName() == EventFunc->GetFName())
			{
				return Existing;
			}
		}
