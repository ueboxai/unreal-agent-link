 ...
 */
static bool CaptureSceneToFile(int32 Width, int32 Height, FString& OutPath, FString& OutError)
{
#if WITH_EDITOR
	if (!GEditor)
	{
	...
	// 真正的原因是两边压根用的不是同一套曝光。
	if (SourceViewport && SourceViewport->ExposureSettings.bFixed)
	{
	...
	// 渲两帧取第二帧：第一帧把眼适应历史建起来（配合上面的极速收敛），
	// 第二帧才是曝光稳定的画面。多渲一帧，实测整体仍在 1 秒内。
	...
	const FString FileName = FString::Printf(TEXT("UAShot_%s.png"), *FDateTime::Now().ToString(TEXT("%Y%m%d_%H%M%S")));
...
void FUAL_EditorCommands::Handle_TakeScreenshot(const TSharedPtr<FJsonObject>& Payload, const FString RequestId)
{
	...
	if (!bShowUI)
	{
		...
		if (CaptureSceneToFile(Width, Height, CapturedPath, CaptureError))
		{
			TSharedPtr<FJsonObject> Data = MakeShared<FJsonObject>();
		...
		// 退回 HighResShot：SceneCapture 失败（比如没打开关卡）时至少还有一条路，
		// 但它需要窗口可见，失败时的提示会说清楚
		UE_LOG(LogUALEditor, Warning,
			TEXT("SceneCapture failed (%s), falling back to HighResShot"), *CaptureError);
	}
