	...
	const TSharedPtr<FJsonObject>& Payload, const FString RequestId)
{
...
#endif
	{
		...
		for (int32 i = 0; i < NodeInputCount; ++i)
		{
			...
			{
				InputName = FString::Printf(TEXT("<%d>"), i);
			}
	...
	auto AddConnection = [&ConnectionsJson](const FString& FromNode, int32 FromOutput,
		const FString& ToNode, const FString& ToInput)
	{
		TSharedPtr<FJsonObject> Conn = MakeShared<FJsonObject>();
		Conn->SetStringField(TEXT("from_node"), FromNode);
...
#endif
	{
		...
		for (int32 i = 0; i < InputCount; ++i)
		{
			...
			AddConnection(NodeIdOf(Input->Expression), Input->OutputIndex,
				NodeIdOf(Expression), InputName);
		}
	}
	...
	// 才能直接拿去当参数用
	struct FRootInput { const TCHAR* Name; FExpressionInput* Input; };
	// UE 5.1+ 起材质主节点的输入挪进了 GetEditorOnlyData()，
	// 直接 Material->BaseColor 编不过。connect_pins 那边也是这么取的。
#if ENGINE_MAJOR_VERSION > 5 || (ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 1)
	UMaterialEditorOnlyData* EditorData = Material->GetEditorOnlyData();
	const FRootInput RootInputs[] = {
		{ TEXT("BaseColor"), EditorData ? &EditorData->BaseColor : nullptr },
		{ TEXT("Metallic"), EditorData ? &EditorData->Metallic : nullptr },
		{ TEXT("Specular"), EditorData ? &EditorData->Specular : nullptr },
		{ TEXT("Roughness"), EditorData ? &EditorData->Roughness : nullptr },
		{ TEXT("EmissiveColor"), EditorData ? &EditorData->EmissiveColor : nullptr },
		{ TEXT("Opacity"), EditorData ? &EditorData->Opacity : nullptr },
		{ TEXT("OpacityMask"), EditorData ? &EditorData->OpacityMask : nullptr },
		{ TEXT("Normal"), EditorData ? &EditorData->Normal : nullptr },
		{ TEXT("WorldPositionOffset"), EditorData ? &EditorData->WorldPositionOffset : nullptr },
		{ TEXT("AmbientOcclusion"), EditorData ? &EditorData->AmbientOcclusion : nullptr }
	};
#else
	const FRootInput RootInputs[] = {
...
#endif
	for (const FRootInput& Root : RootInputs)
	{
		if (Root.Input && Root.Input->Expression)
		{
			AddConnection(NodeIdOf(Root.Input->Expression), Root.Input->OutputIndex,
				TEXT("Material"), Root.Name);
		}
	}
	...
	const TSharedPtr<FJsonObject>& Payload, const FString RequestId)
{
	...
	else
	{
		...
		{
			if (!Expression) continue;
			const FString NodeId = FString::Printf(TEXT("%s_%d"), *Expression->GetClass()->GetName(), TargetIndex++);
			if (NodeId == TargetNode || Expression->GetName() == TargetNode)
			{
		...
		for (int32 i = 0; i < InputCount; ++i)
		{
			...
			AvailableInputs.Add(InputName.IsEmpty() ? FString::Printf(TEXT("<%d>"), i) : InputName);
		...
		// 源节点的输出序号。绝大多数节点只有一个输出；ComponentMask 这类
		// 多输出节点可以用 source_pin 传序号
